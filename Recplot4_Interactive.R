...
get_names <- function(prefix){
  
  names <- unique(fread(paste0(prefix, ".lim"), sep = "\t", quote = "", select = c(1))$V1)
...
recplot_suite_mags <- function(prefix){
  
  mag_list <- get_names(prefix)
  
  LR <- get_lim_rec(prefix, MAG = mag_list[1])
  ...
  ui <- fluidPage(
    ...
    sidebarPanel(
      ...
      selectInput("which_mag", "Current MAG", choices = list("placeholder" = 1), selected = 1),
  ...
  server <- function(input,output, session){
    ...
    updateSelectInput(session, "which_mag", choices = mag_list)
    
    #input$which_mag <- mag_list[1]
    
    ranges <- reactiveValues(x = NULL, y = NULL)

    dat <- reactive({
      
      LR <<- get_lim_rec(prefix = prefix, MAG = input$which_mag)
      ...
      updateNumericInput(session, "binSz", max = max(LR[[1]]$V3, na.rm = T))
      updateNumericInput(session, "genome_region_min", max = pos_max-1)
      updateNumericInput(session, "genome_region_max", value = pos_max, max = pos_max)
...
#Example usage - you put the directory and prefix here
#setwd("C:/Users/Kenji/Desktop/Recplot4")
#recplot_suite_mags(prefix = "full_mass_plot")
