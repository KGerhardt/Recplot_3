  ...
  contigs <- data.table(pos =  LR[[1]]$V2, contig = LR[[1]]$V1[findInterval(seq_breaks*bp_div, LR[[1]]$V2)])
    ...
    ui <- fluidPage(
      ...
      sidebarPanel(
                   ...
                   numericInput("binSz", "Pos. Bin Size:", 1000, min = 1, max = max(LR[[1]]$V3, na.rm = T), step = 100),
                   numericInput("ani_step", "ANI Bin Size:", 0.5, min = 0.05, max = 5, step = 0.1),
                   numericInput("in_group_min", "Lower ANI Bound:", 95, min = 70, max = 100, step = 1),
                   numericInput("genome_region_min", "Start Pos.", 0, min = 0, max = pos_max-1),
                   numericInput("genome_region_max", "End Pos.", pos_max, min = 1, max = pos_max),
      ...
      mainPanel(
                tabsetPanel(type = "tabs",
                  tabPanel("Overview", column(8, 
                plotOutput("seq_depth_chart"),
                plotOutput("read_rec_plot"
                           )),
                column(4,
                      plotOutput("seq_depth_hist"),
                      plotOutput("bp_hist"))),
  ...
  server <- function(input,output){
    ...
    dat <- reactive({
      ...
      output$Plotly_read_rec <- renderPlotly({
        ...
        p <- ggplot(base, aes(x=Pos_In_Genome/bp_div, y=ANI_Range, fill=log10(Bin_Count), text = paste0("Contig: ", contigs$contig[(findInterval(base$Pos_In_Genome, contigs$pos))], "\nPos. in Contig: ", (Pos_In_Genome + 1 - contigs$pos[(findInterval(base$Pos_In_Genome, contigs$pos))]), "\nBin Count: ", base$Bin_Count, "\nANI Range: ", base$ANI_Range-input$ani_step, "-", base$ANI_Range)))+ 
          scale_fill_gradient(low = "white", high = "black",  na.value = "#EEF7FA")+
          ylab("Percent Identity") +
          xlab(paste("Position in Genome", bp_unit)) +
          scale_y_continuous(expand = c(0, 0)) +
      ...
      output$Plotly_SD <- renderPlotly({
        ...
        p2 <- ggplot(depth_data, aes(x = Pos_In_Genome, y = count, colour=group_label, group = group_label,
                                     text = paste0("Contig: ", contigs$contig[(findInterval(depth_data$Pos_In_Genome*bp_div, contigs$pos))], 
                                                  "\nPos. in Contig: ", (depth_data$Pos_In_Genome*bp_div + 1 - contigs$pos[(findInterval(depth_data$Pos_In_Genome*bp_div, contigs$pos))]), 
                                                  "\nAvg. depth on interval: ", round(depth_data$count, 2))))+
          geom_step(alpha = 0.75) +
          scale_y_continuous(trans = "log10") +
          scale_x_continuous(expand=c(0,0))+
          theme(legend.position = "none", panel.border = element_blank(), panel.grid.major = element_blank(),
                panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), panel.background = element_blank(), axis.title.x = element_text("Pos. In Genome"), axis.text.y = element_blank()) +
          scale_color_manual(values = group.colors) +
          ylab("Depth")
