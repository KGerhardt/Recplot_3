...
def sqldb_creation(contigs, mags, sample_reads, map_format, database):
	...
	for sample_name in sample_reads:
		...
		shortened_name = os.path.normpath(sample_name)
		shortened_name = shortened_name.split("\\")
		shortened_name = shortened_name[len(shortened_name)-1]
		shortened_name = shortened_name.split("/")
		shortened_name = shortened_name[len(shortened_name)-1]
		
		sampleid_to_sample[sample_id] = shortened_name
		sample_number += 1
...
def add_sample(database, new_mapping_files, map_format):
	...
	for new_sample in new_mapping_files:
		...
		shortened_name = os.path.normpath(new_sample)
		shortened_name = shortened_name.split("\\")
		shortened_name = shortened_name[len(shortened_name)-1]
		shortened_name = shortened_name.split("/")
		shortened_name = shortened_name[len(shortened_name)-1]
		
		new_sample = shortened_name
