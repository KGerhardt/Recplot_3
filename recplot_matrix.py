...
#! DATABASE CREATION
def blast_like_to_db(contigs, mags, reads, format, database, overwrite):
    # Create or open database
    conn = sqlite3.connect(database)
    cursor = conn.cursor()
    # Create tables (drop and create if overwrite equals True)
    if overwrite == True:
        cursor.execute('DROP TABLE IF EXISTS rec_table')
        cursor.execute('DROP TABLE IF EXISTS contig_info')
        cursor.execute('DROP TABLE IF EXISTS gene_info')
        cursor.execute('CREATE TABLE rec_table ("mag", "contig", "identity", "start", "stop")')
        cursor.execute('CREATE INDEX mag_index on rec_table (mag)')
        cursor.execute('CREATE TABLE mag_table ("mag", "contig", "len_contig")'
        cursor.execute('CREATE TABLE gene_table ("mag", "contig", "gene", "gene_start", "gene_stop")'
    else:
        cursor.execute('CREATE TABLE IF NOT EXISTS rec_table ("mag", "contig", "identity", "start", "stop")')
        cursor.execute('CREATE INDEX mag_index on rec_table (mag)')
        cursor.execute('CREATE TABLE IF NOT EXISTS mag_table ("mag", "contig", "len_contig")'
        cursor.execute('CREATE TABLE IF NOT EXISTS gene_table ("mag", "contig", "gene", "gene_start", "gene_stop")'

    # Get contig sizes
    contig_size = read_contigs(contigs)
    # Get MAG - contig information
    get_mags(mag_file)
    # Read read mapping file
    with open(reads) as input_reads:
        record_counter = 0
        records = []
        if format == "blast":
            for line in input_reads:
                ...
                if counter == 500000:
                    cursor.executemany('INSERT INTO rec_table VALUES(?, ?, ?, ?, ?)', records)
                    conn.commit()
                    counter = 0
                    records = []
                if line.startswith("#"):
...
def opts():
    ...
    parser.add_option("-d", "--database", dest="sql_database", action='store', help = "SQLite database to create or update")
